...
gray_image_blr =  cv2.GaussianBlur(gray_image, (25,25),0)



# Calculate histogram (only for visualization):
hist = cv2.calcHist([gray_image_blr], [0], None, [256], [0, 256])

# Threshold the image aplying Otsu's algorithm:
ret1, th1 = cv2.threshold(gray_image_blr, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
